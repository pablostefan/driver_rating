...
class DriverRatingHistoricCardWidget extends StatelessWidget {
  final Driver driver;
  final Function(Driver) onTapDriver;

  const DriverRatingHistoricCardWidget({super.key, required this.driver, required this.onTapDriver});

  String get _package => "module_assets";

  TextStyle get _textStyle => const TextStyle(color: DriverRatingColors.monoWhite);

  List<Rating> get _historicRatings => driver.ratings.where((element) => element.userRating).toList();
    ...
    return DriverRatingGestureDetectorWidget(
        ...
        child: Container(
            ...
            child: Column(children: [
              ...
              Padding(
                  padding: const EdgeInsets.only(top: DriverRatingDimens.xxs),
                  child: Text(TranslateHome.strings.yourReviews).bodyMediumSemiBold(style: _textStyle)),
              ListBody(children: _historicRatings.map((rating) => DiverRatingCardWidget(rating: rating)).toList())
            ])));
  }
}
